...
where not exists (select project_id
                 from project p
                 where budget=10000 and not exists (select employee_id
                                                    from works_on
...
where (select count(project_id)
      ...
      where budget=10000) = (select count(project_id)
                            ...
                            where budget=10000 and employee_id=A.employee_id);
--var 3                           
select distinct employee_id
from works_on w
where not exists( select project_id
...
-- var 4
select distinct employee_id from works_on where employee_id not in (
select employee_id from
                  
(select employee_id , t.project_id
from works_on, (select project_id 
                from project 
...
--1 . angajatul meu este afisat daca nu exista proiect demarat ... la care sa nu fi lucrat
select distinct employee_id
from works_on w
where not exists (select project_id
                  from project p
...
--3
--multiem angajati care lucreaza la proiect (a doua dupa minus)

--multime angajati 2 joburi
select project_id 
from project p
where not exists
(select employee_id
...
group by j.employee_id, x.job_id
having count(distinct j.job_id) + 
case when x.job_id in (select job_id from job_history where employee_id=j.employee_id) then 0 else 1 end > 2 ; --1 provine din jobul curent
...
stevenking_old as (select employee_id
                    from stevenking
                    where hire_date = (select min(hire_date) from stevenking))
select employee_id, last_name||first_name, job_id, hire_date, level
from employees
where to_char(hire_date, 'yyyy') <> 1970
start with employee_id in (select employee_id from stevenking_old)
