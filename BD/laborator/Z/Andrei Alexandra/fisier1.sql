...
values(60,'Analiza');
commit;
--pb 27 lab 8 nu putem pt ca avem angajati in acel departament
delete from departamente_pnu
where cod_dep=20;
--din dep 60 se poate
delete from departamente_pnu
...
--pb 31 lab 8
alter table angajati_pnu
drop constraint fk_cod_depp;

alter table angajati_pnu add constraint fk_cod_depp
...
rollback;
--pb 33 lab 8
alter table departamente_pnu add constraint fk_cod_dr
foreign key(cod_director) references angajati_pnu(cod_ang) on delete set null;
--pb 34 lab 8 nu pot sa sterg daca el are subalterni
...
rollback;
--inainte de a face un alter table el face commit
--pb 1 lab 9
create global temporary table temp_tranz_pnu
...
--o conditie necesara sa pot efectua INSERT intr-un VIEW este sa am toate coloanele not null din tabelul de baza
--UPDATE-ul modifica linii doar din VIEW si doar coloane care exista in VIEW
--DELETE modifica linii din VIEW
--pe orice coloana "calculata" nu se pot efectua operatii LMD(in tabelul de baza nu exista coloana calculata)
update viz_emp30 
...
as select employee_id, last_name, email, salary,job_id,hire_date,salary*12 sal_anual
from emp_pnu
where department_id=50;
...
create or replace view viz_emp50
as select employee_id, last_name, email, salary,job_id,hire_date,salary*department_id sal_anual
from emp_pnu
where department_id=50;
delete from viz_emp50
